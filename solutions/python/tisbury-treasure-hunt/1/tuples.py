...
def compare_records(azara_record, rui_record):
    ...
    new_record_coordinates = tuple(azara_record[1])
    return new_record_coordinates == rui_record[1]
...
def create_record(azara_record, rui_record):
    ...
    if tuple(azara_record[1]) == rui_record[1]:
        combine = azara_record + rui_record
    else:
        combine = "not a match"
...
def clean_up(combined_record_group):
    ...
    report = ""
    for indice,val in enumerate(combined_record_group):
        new = tuple([val[0], val[2], val[3], val[4]])
        report = report + str(new) + "\n"
    return report
